...
- **Paragraph rendering**: dialogue is written with speech in DOUBLE quotes; everything outside them is action. The engine (`printNpcBeats` in ui.js) packs beats into compact prose paragraphs — each ACTION beat opens a new paragraph and the speech that follows stays attached inline. Actions render in the body colour (no gray, no italic); speech renders bold cyan in typographic quotes. Attributions ("he says") are avoided; write full-sentence beats. A lint checks every dialogue string has an even number of `"`. Budget: aim ≤3 paragraphs for topics/returns; first-meet greetings and Avalonne's lesson may run 4–5.
- **Conversation prompt**: one short line — "(ask [topic], or 'goodbye' to leave)" — shown only until the player first uses `ask` successfully (`GS.flags.usedAsk`); after that, just the Topics line.
...
### The Knight → Avalonne (combat teacher)
- The wounded knight is bleeding out: **spare, urgent dialogue, no lore, no name** — only two topics (`wound`, `help`) and a plea for mending. He does NOT ramble about the Toll or "the body remembers."
- The **Porter gives a Minor Healing Tome on first conversation** (`givesOnFirstMeet` + `giftText` in npcs.js; one-time via `GS.flags.gift_porter`). `use minor healing tome` next to the knight heals him (routes to doGive); used elsewhere it's a one-shot +25 self-heal.
...
- **Counter** (the reward for a successful dodge of a lesser blow, or a read-through feint): your next attack can't miss and deals ×1.5, but the enemy is NOT skipped — it acts on its next telegraph. So dodging is worth a turn (you avoid damage and hit harder) without being a free stagger-loop. Consumed only when it actually powers an attack (attacking a staggered foe keeps the counter for later).
- **Earned openings are never robbed by RNG**: the adaptation slip cannot trigger while the enemy is staggered or a counter is loaded — a correct read always pays what it promised.
